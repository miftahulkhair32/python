# klo mau di jalan pke python 2 harus Tkinter
from tkinter import *
...
class app(Frame):
    def __init__(self):
        Frame.__init__(self)
        self.option_add('*Font', 'nimbus 20 bold')
        self.pack(expand=YES, fill=BOTH)
        self.master.title('kakulator BUATAN SENDIRI OW')

        display = StringVar()
        Entry(self, relief=RIDGE, textvariable=display, justify='right',        bd=30, bg="powder blue").pack(side=TOP, expand=YES,             fill=BOTH)
        for clearBut in (['HAPUS TOTAL'],['HAPUS TOTAL JUGA']):
